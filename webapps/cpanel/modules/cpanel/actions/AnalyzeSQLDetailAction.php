 ...
 * @package actions
 */
class AnalyzeSQLDetailAction extends PHI_Action
{
  public function execute()
  {
    ...
    if ($statementInfo) {
      ...
      try {
        $sender = new PHI_HttpRequestSender('http://sqlformat.appspot.com/format/');
        $sender->setReadTimeout(3);
        $sender->setRequestMethod(PHI_HttpRequest::HTTP_POST);
      ...
      if (strcasecmp(mb_substr($statement, 0, 6, $encoding), 'SELECT') === 0) {
        ...
        $pos = PHI_StringUtils::searchIndex($statement, 'SET', 0, '\'');
        $buffer = mb_substr($statement, 6, $pos - 6, $encoding);
        $sql = sprintf('EXPLAIN SELECT * FROM %s', $buffer);

        if (($pos = PHI_StringUtils::searchIndex($statement, 'WHERE', 0, '\'')) !== FALSE) {
        ...
        $pos = PHI_StringUtils::searchIndex($statement, 'FROM', 0, '\'');
        $buffer = mb_substr($statement, $pos + 4, $encoding);

        if (($pos = PHI_StringUtils::searchIndex($buffer, 'USING', 0, '\'')) !== FALSE) {
