<?php
require PHI_LIBS_DIR . '/cache/PHI_Cache.php';
require PHI_LIBS_DIR . '/cache/PHI_CacheManager.php';
require PHI_LIBS_DIR . '/cache/PHI_YAMLCache.php';

require PHI_LIBS_DIR . '/kernel/container/PHI_DIContainer.php';
require PHI_LIBS_DIR . '/kernel/container/PHI_DIContainerFactory.php';

require PHI_LIBS_DIR . '/util/PHI_ParameterHolder.php';
require PHI_LIBS_DIR . '/util/common/PHI_ArrayUtils.php';
require PHI_LIBS_DIR . '/util/common/PHI_CommonUtils.php';
require PHI_LIBS_DIR . '/util/config/PHI_Config.php';
require PHI_LIBS_DIR . '/util/common/PHI_FileUtils.php';
require PHI_LIBS_DIR . '/util/common/PHI_StringUtils.php';
...
class PHI_ClassLoader
{
   ...
   * @var array
   */
  private static $_searchPaths = array(PHI_LIBS_DIR);
  ...
  public static function initialize()
  {
    ...
    self::$_instance = new PHI_ClassLoader();

    // オートローダの登録
    spl_autoload_register(array('PHI_ClassLoader', 'loadByName'));

    if (PHI_BootLoader::isBootTypeCommand()) {
      self::$_isBootTypeCommand = TRUE;
      self::$_cache = PHI_CacheManager::getInstance(PHI_CacheManager::CACHE_TYPE_NULL);

    } else {
      self::$_searchPaths[] = APP_ROOT_DIR . '/libs';
      self::$_cache = PHI_CacheManager::getInstance(PHI_CacheManager::CACHE_TYPE_FILE);
    }
  }
  ...
  public static function addSearchPath($path)
  {
    ...
    try {
      $path = PHI_FileUtils::buildAbsolutePath($path);
   ...
   * @throws PHI_IOException クラスパスが見つからない場合に発生。(throw が TRUE の場合のみ)
   */
   ...
   * @throws PHI_IOException クラスが見つからない場合に発生。
   */
  public static function loadByName($className)
  {
    ...
    if (!in_array($className, self::$_loadedClasses)) {
      ...
      if (!$hasLoadVendorClass) {
        ...
        } else if (substr($className, 0, 6) !== 'PHI_' && !is_file($findPath)) {
  ...
  private static function searchFile($searchPath)
  {
    ...
    $files = PHI_FileUtils::search($searchPath, '/\.php/', $options);
    $classes = array();
  ...
  public function __destruct()
  {
    ...
    if ($classes) {
      $autoloaders = PHI_ArrayUtils::merge($classes, self::$_preWriteAutoloaders);
    } else {
