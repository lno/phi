 ...
 */
class PHI_ErrorHandler
{
  /**
   * アプリケーション内で発生した PHP エラーを {@link ErrorException} オブジェクトに変換し、{@link PHI_ExceptionHandler::invoker()} に例外を送信します。
   *
  ...
  public static function handler($type, $message, $file, $line, array $context = array())
  {
    ...
    if (error_reporting() != 0) {
      $catchLevel = PHI_Config::getApplication()->getInt('error.catchLevel');
  ...
  public static function invokeFatalError($type, $message, $file, $line)
  {
    ...
    if (PHI_BootLoader::isBootTypeWeb()) {
      ...
      if (!is_file($path)) {
        $path = PHI_ROOT_DIR . '/skeleton/templates/fatal_error.php';
      }
      ...
      if (is_file($file)) {
        $variables['code'] = PHI_DebugUtils::syntaxHighlight(PHI_FileUtils::readFile($file), $options);

      // PHI_PHPStringParser::parse() で Fatal エラーが発生 (eval() エラー)
      } else {
        $registry = PHI_Object::getRegistry();

        if ($registry->hasName('parseCode')) {
          $variables['code'] = PHI_DebugUtils::syntaxHighlight($registry->get('parseCode')->getCode(), $options);
        }
      }
