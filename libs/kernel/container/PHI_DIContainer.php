...
class PHI_DIContainer
{
  ...
  {
    $this->_config = PHI_Config::getDIContainer();
  }
   ...
   * @param PHI_ParameterHolder $attributes コンポーネント属性。
   ...
   * @todo attributes を PHI_ParameterHolder に変更。
   */
  public function componentRegister($componentName, array $attributes, $autoload = TRUE)
  {
    ...
    if (isset($attributes['class'])) {
      ...
      if (isset($attributes['path'])) {
        PHI_ClassLoader::loadByPath($attributes['path'], $className);

      } else if ($autoload) {
        PHI_ClassLoader::loadByName($className);
      }
      ...
      if (isset($attributes['includes'])) {
        foreach ($attributes['includes'] as $path) {
          ...
          require_once PHI_FileUtils::buildAbsolutePath($path);
        }
      }

      // コンストラクタインジェクション
      $constructor = PHI_ArrayUtils::find($attributes, 'constructor', array());
  ...
  private function executeMethodInjection(ReflectionClass $reflection, $component, array $methods)
  {
    foreach ($methods as $methodName => $arguments) {
      ...
      if ($reflection->hasMethod($methodName)) {
        // DI 設定で引数が未指定
        if (PHI_StringUtils::nullOrEmpty($arguments)) {
  ...
  public function getComponent($componentName)
  {
    ...
    if (isset($this->_components[$componentName])) {
      ...
      if ($attributes) {
        ...
        $classPath = PHI_ClassLoader::findPath($className);
