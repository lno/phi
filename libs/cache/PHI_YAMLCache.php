 ...
 * @package cache
 */
class PHI_YAMLCache extends PHI_Object
{
  /**
  ...
  private function set($yamlPath, $data)
  {
    ...
    if (!is_dir($cacheDirectory)) {
      PHI_FileUtils::createDirectory($cacheDirectory, 0777);
    ...
    $code = sprintf("<?php\n"
      ."// This script was auto-generated from PHI_YAMLCache::set().\n"
      ."// date: %s\n\n"
      ."PHI_YAMLCache::load('%s', %s);\n",
      date('m/d/Y H:i:s'),
      $cachePath,
   ...
   * @throws PHI_IOException ファイルが見つからない場合に発生。
   */
  private function getFromRawData($yamlPath, $callback, $hasExtensionFile)
  {
    ...
    if ($hasExtensionFile) {
      ...
      if (is_file($extensionPath)) {
        ...
        $data = PHI_ArrayUtils::merge($data1, $data2);
   ...
   *   {@link PHI_ConfigCompiler} のコールバックメソッドを指定可能。
   * @param bool $hasExtensionFile ホスト名による拡張ファイルが定義されている場合に TRUE を指定。
   * @param bool &$readFromCache ファイルがキャッシュから読み込まれた場合は TRUE を返します。
   * @return array データ配列を返します。
   * @throws PHI_IOException yamlPath が見つからない場合に発生。
   */
