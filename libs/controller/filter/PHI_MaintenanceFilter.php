 ...
 * <i>その他に指定可能な属性は {@link PHI_Filter} クラスを参照して下さい。</i>
 *
 * @package controller.filter
 */
class PHI_MaintenanceFilter extends PHI_Filter
{
  /**
  ...
  public function isServiceAvailable()
  {
    ...
    if ($periodic) {
      if (!$periodic->hasName('beginTime')) {
        throw new PHI_ConfigurationException('\'periodic.beginTime\' attribute is not defined.');
      }

      if (!$periodic->hasName('endTime')) {
        throw new PHI_ConfigurationException('\'periodic.endTime\' attribute is not defined.');
      }
      ...
      if ($type === 'daily') {
        ...
        if (!$periodic->hasName('target')) {
          throw new PHI_ConfigurationException('\'periodic.target\' attribute is not defined.');
        }
