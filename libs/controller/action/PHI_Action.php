 ...
 * コントローラはリクエストから適切な PHI_Action を選択して処理を開始します。
 *
 * @package controller.action
 */
abstract class PHI_Action extends PHI_WebApplication
{
  /**
   * アクションパス。
   * @var string
   ...
   *
   * @return PHI_FrontController フロントコントローラオブジェクトを返します。
   */
   ...
   * execute() メソッドは戻り値として {@link PHI_View} 定数を返す必要があります。
   *
   * @return string フォワードするビューを返します。戻り値が不明な場合は {@link PHI_View::SUCCESS} が返ります。
   */
   ...
   *   メソッドがオーバーライドされていない場合は {@link PHI_View::ERROR} にフォワードされます。
   */
   ...
   *
   * @return string フォワード先のビューを返します。
   *   メソッドがオーバーライドされていない場合は {@link PHI_View::SAFETY_ERROR} にフォワードされます。
   */
  public function safetyErrorHandler()
  {
    return PHI_View::SAFETY_ERROR;
  }
