<?php
require PHI_LIBS_DIR . '/controller/router/PHI_Route.php';
 ...
 */
class PHI_RouteResolver extends PHI_Object
{
  /**
   * リクエストオブジェクト。
   * @var PHI_HttpRequest
   ...
   * コンストラクタ。
   *
   * @param PHI_HttpRequest $request HTTP リクエストオブジェクト
   */
  public function __construct(PHI_HttpRequest $request)
  {
    $this->_applicationConfig = PHI_Config::getApplication();
    $this->_routesConfig = PHI_Config::getRoutes();
  ...
  public function isAllowPackage($packageName)
  {
    ...
    foreach ($this->_routesConfig as $routeName => $attributes) {
      if (isset($attributes['packages'])) {
        $isValid = PHI_Action::isValidPackage($packageName, $attributes['packages']);
   ...
   *
   * @return PHI_Route 適切なルートが見つかった場合にルート情報を格納した PHI_Route オブジェクトを返します。
  ...
  public function connect()
  {
      ...
      if (preg_match($routeConfig->get('regexp'), $this->_requestUri)) {
        ...
        for ($i = 0; $i < $j; $i++) {
          ...
          switch ($segmentId) {
            ...
            case ':action':
              $pathHolder['action'] = PHI_StringUtils::convertPascalCase($segmentValue);
              break;
    ...
    if ($detectedRoute) {
      ...
      $route = new PHI_Route($pathHolder);
    }
  ...
  /**
   * @param PHI_ParameterHolder $accessConfig
   * @return bool
   */
  private function isAllowNetwork(PHI_ParameterHolder $accessConfig)
  {
    ...
    if ($allows) {
      ...
      foreach ($allows as $allow) {
        if (PHI_NetworkUtils::hasContainNetwork($allow, $remoteAddress)) {
          $result = TRUE;
          break;
        }
      }
   ...
   * @throws PHI_ForwardException ルートパスの生成に必要なホルダが不足している場合に発生。
   * @throws PHI_ConfigurationException 指定されたルートがルート設定ファイル定義されていない場合に発生。
   */
