 ...
 *   {appender_name}:
 *     # ロガークラス名。
 *     class: PHI_LoggerMailAppender
 *
 *     # メール送信オプション。未指定の場合は application.yml の 'mail' 属性が参照される。
 *     # 設定可能な属性については {@link PHI_MailSender} のクラスリファレンスを参照。
 *     mail:
 *
 ...
 */
class PHI_LoggerMailAppender extends PHI_LoggerAppender
{
  /**
   * メールオブジェクトを生成します。
   *
   * @param PHI_ParameterHolder $holder パラメータホルダ。
   * @return PHI_MailSender PHI_MailSender のインスタンスを返します。
   * @throws PHI_ParseException 'from' または 'to' のアドレスが空の場合に発生。
   */
  protected function getMailSender()
  {
    ...
    if (!isset($instances[$this->_appenderId])) {
      ...
      if ($from === NULL) {
        throw new PHI_ParseException('Required to send mail "From" header is undefined.');
      }

      if ($to === NULL) {
        throw new PHI_ParseException('Required to send mail "To" header is undefined.');
      }

      $config = $holder->getArray('mail');

      $mail = new PHI_MailSender($config);
      $mail->setFrom($from, $fromName);
      $mail->setRecipients(PHI_MailSender::RECIPIENT_TYPE_TO, $to);
   ...
   * @param int $type ログタイプを PHI_Logger::MASK_* 定数で指定。
  ...
  public function getSubjectFormat($className, $type, $message)
  {
    ...
    return PHI_StringUtils::truncate($format, 64);
  }
  ...
  public function getBodyFormat($className, $type, $message)
  {
    if ($message instanceof Exception) {
      ...
      if ($exception !== 'simple') {
        ...
        if ($exception === 'detail') {
          ...
          foreach ($array as $name => $value) {
            ...
            $name,
            $separator,
            PHI_CommonUtils::convertVariableToString($value));
          }
        }
      }
      ...
      $format = sprintf("%s\n\n(%s)", PHI_CommonUtils::convertVariableToString($message), $date);
    }
  ...
  protected function maskProtectKeywords(array $variables, array $keywords, $protectMask)
  {
    foreach ($keywords as $keyword) {
      PHI_ArrayUtils::replaceValueWithCallback($variables, $keyword, function($value) use ($protectMask) {
        return $protectMask;
      });
    }
