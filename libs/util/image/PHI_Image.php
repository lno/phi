 ...
 * @package util.image
 */
class PHI_Image extends PHI_Object
{
  /**
   * GIF 定数。
   */
  ...
  /**
   * {@link PHI_ImageDelegate} オブジェクト。
   * @var PHI_ImageDelegate
   */
  private $_delegate;
   ...
   * @return PHI_Image PHI_Image オブジェクトを返します。
   */
  public function setDelegate(PHI_ImageDelegate $delegate)
  {
   ...
   * デリゲートオブジェクトを取得します。
   *
   * @return PHI_ImageDelegate PHI_ImageDelegate を実装したイメージオブジェクトのインスタンスを返します。
   */
   ...
   * @throws PHI_ParseException ファイルを識別できない場合に発生。
   */
   ...
   * @throws PHI_ParseException 解析不能な形式が指定された場合に発生。(解析可能な形式は IMAGE_TYPE_* 定数を参照)
   */
   ...
   * @param string $data
   * @return int
   * @throws PHI_UnsupportedException
   */
  public static function parseImageTypeFromBinary($data)
  {
    ...
    if (strncmp("\x89PNG\x0d\x0a\x1a\x0a", $data, 8) === 0) {
      ...
      throw new PHI_DataNotFoundException('Image data not found.');
   ...
   * データの圧縮は {@link save()}、または {@link render()} メソッドで実行されますが、指定サイズ以下に制限できなかった場合は {@link PHI_ImageException} をスローします。
   *
   ...
   * @return PHI_Image PHI_Image オブジェクトを返します。
   * @throws PHI_ImageException イメージの変換に失敗した際に発生。
   */
  public function convertFormat($toType)
  {
    ...
    if (!$result) {
      ...
      throw new PHI_ImageException($message);
    }
   ...
   * @param PHI_ImageColor $fillColor 余白を塗りつぶす色。
   * @return PHI_Image PHI_Image オブジェクトを返します。
   */
  public function trim($newWidth, $newHeight, $horizontal = 50, $vertical = 50, $fillColor = NULL)
  {
    ...
    if ($fillColor === NULL) {
      $fillColor = PHI_ImageColor::createFromRGB(0, 0, 0);
    }
   ...
   * @param PHI_ImageColor $fillColor {@link trim()} メソッドを参照。
   * @return PHI_Image PHI_Image オブジェクトを返します。
   */
  ...
  private function flush($path = NULL)
  {
    ...
    } else if ($type == PHI_Image::IMAGE_TYPE_JPEG) {
      $currentQuality = 75;
      $degradation = 5;
      $tempPath = NULL;
  ...
  {
    $buffer = PHI_Helper::buildTagAttribute($attributes, FALSE);
   ...
   * @return PHI_Image PHI_Image オブジェクトを返します。
   */
//  public static function read($path) // TODO TEST
   ...
   * イメージのコピーを生成します。
   *
   * @return PHI_Image イメージのコピーを返します。
   */
