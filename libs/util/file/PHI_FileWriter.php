 ...
 * @package util.file
 */
class PHI_FileWriter extends PHI_Object
{
  /**
   * ファイルハンドラ。
   * @var resource
  ...
  {
    $this->_inputEncoding = PHI_Config::getApplication()->get('charset.default');
  ...
  {
    if (PHI_FileUtils::isAbsolutePath($writePath)) {
   ...
   *
   * @see PHI_LogWriter::write()
   * @see PHI_LogWriter::setLinefeed()
   */
  ...
  protected function buildOutputData($data = NULL)
  {
    ...
    if ($this->_convertLinefeed) {
      $data = PHI_StringUtils::replaceLinefeed($data, $this->_linefeed);
    }
  ...
  /**
   * @param string $data
   * @throws PHI_IOException
   */
  private function realWrite($data)
  {
    if ($this->_path === NULL) {
      throw new PHI_IOException('Write path is not set.');
