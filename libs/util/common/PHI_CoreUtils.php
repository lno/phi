...
class PHI_CoreUtils
{
 ...
 /**
  ...
  public static function clearCache()
  {
    ...
    PHI_FileUtils::deleteDirectory($cacheDir, FALSE);

    if (!is_dir($cacheDir)) {
      PHI_FileUtils::createDirectory($cacheDir, 0775);
    }

    $autoloadDir = $cacheDir . '/file';
    PHI_FileUtils::createDirectory($autoloadDir, 0775);

    $templatesDir = $cacheDir . '/templates';
    PHI_FileUtils::createDirectory($templatesDir, 0775);

    $yamlDir = $cacheDir . '/yaml';
    PHI_FileUtils::createDirectory($yamlDir, 0775);
  }
  ...
  public static function addCommand($commandName, $packageName = '/')
  {
    ...
    if (!is_dir($writeDirectory)) {
      PHI_FileUtils::createDirectory($writeDirectory);
    }

    // クラスパスの生成
    $commandClassName = PHI_StringUtils::convertPascalCase($commandName);
   ...
   * @throws PHI_ParseException 使用できない文字が含まれる場合に発生。
   * @throws PHI_IOException モジュールの作成に失敗した場合に発生。
   */
  public static function addModule($moduleName)
  {
    ...
    PHI_FileUtils::createDirectory($modulePath);

    $skeletonPath = PHI_SKELETON_DIR . '/blank_module';
    $options = array('recursive' => TRUE);

    PHI_FileUtils::copy($skeletonPath, $modulePath, $options);
    $deployFiles = PHI_FileUtils::search($modulePath, '/.*/', array('directory' => TRUE));
    ...
    $toPath = sprintf('%s%smodules%s%s%stemplates%sindex%s',
      ...
      DIRECTORY_SEPARATOR,
      DIRECTORY_SEPARATOR,
      PHI_Config::getApplication()->getString('view.extension'));
   ...
   * @throws PHI_ParseException モジュールやテーマに指定不可能な文字が含まれる場合に発生。
   */
  public static function addTheme($basePath, $themeName, array $moduleNames)
  {
    ...
    PHI_FileUtils::createDirectory($themePath, 0775, TRUE);
    ...
    foreach ($moduleNames as $moduleName) {
      ...
      foreach ($createList as $path) {
        PHI_FileUtils::createDirectory($path, 0755, TRUE);
    ...
    $copyFiles = array(
      PHI_ROOT_DIR . '/skeleton/blank_application/templates' =>
        sprintf('%s%stemplates', $themePath, DIRECTORY_SEPARATOR)
    );

    foreach ($copyFiles as $from => $to) {
      PHI_FileUtils::copy($from, $to, array('recursive' => TRUE));
    }

    $deployFiles = PHI_FileUtils::search($themePath, '/.*/', array('directory' => TRUE));
   ...
   * @throws PHI_ParseException 使用できない文字が含まれる場合に発生。
   * @throws PHI_IOException ファイルの作成が失敗した場合に発生。
   */
  public static function addAction($moduleName, $actionName, $packageName = '/')
  {
    ...
    $actionName = PHI_StringUtils::convertPascalCase($actionName);
    $actionFile = $actionName . 'Action.php';

    $actions = PHI_FileUtils::search($moduleDirectory, $actionFile);
    ...
    if (!is_dir($toDirectory)) {
      PHI_FileUtils::createDirectory($toDirectory);
    }
    ...
    $templateName = PHI_StringUtils::convertSnakeCase($actionName);
  ...
  {
    $string = file_get_contents(PHI_ROOT_DIR . '/VERSION');
  ...
  public static function getModuleNames()
  {
    ...
    if (is_dir($modulePath)) {
      ...
      foreach ($paths as $path) {
        ...
        if (preg_match(PHI_CoreUtils::REGEXP_MODULE, $path)) {
          $modules[] = $path;
        }
      }
    }
  ...
  /**
   * フレームワークのライブラリ (PHI_LIBS_DIR) に含まれる全てのファイルパスを取得します。
   *
  ...
  public static function getCoreClasses()
  {
    ...
    $files = PHI_FileUtils::search(PHI_LIBS_DIR, '/^[\w]+\.php$/', $options);
