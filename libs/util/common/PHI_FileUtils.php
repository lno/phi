...
class PHI_FileUtils
{
   ...
   * @throws PHI_IOException 解析できないパス文字列が渡された場合に発生。
   */
  public static function isAbsolutePath($path)
  {
    $result = FALSE;

    if (PHI_StringUtils::nullOrEmpty($path)) {
      throw new PHI_IOException('Specified path is NULL or empty.');
    }
   ...
   * @throws PHI_IOException 対象ディレクトリが存在しない場合に発生。
   * @see 1.60
   */
  public static function search($searchPath, $searchFile, array $options = array())
  {
    $array = array();
    $array['excludes'] = PHI_ArrayUtils::find($options, 'excludes', array());
    $array['basePath'] = PHI_ArrayUtils::find($options, 'basePath');
    $array['separator'] = PHI_ArrayUtils::find($options, 'separator', DIRECTORY_SEPARATOR);
    $array['searchFileIsRegexp'] = FALSE;
    $array['index'] = 0;
    $array['hidden'] = PHI_ArrayUtils::find($options, 'hidden', TRUE);
    $array['pattern'] = PHI_ArrayUtils::find($options, 'pattern');
    $array['directory'] = PHI_ArrayUtils::find($options, 'directory', FALSE);
   ...
   *
   * @param string $searchPath PHI_FileUtils::search() 関数を参照。
   * @param string $searchFile PHI_FileUtils::search() 関数を参照。
   * @param array $options PHI_FileUtils::search() 関数を参照。
   ...
   * @throws PHI_IOException ディレクトリが既に存在する (かつ、force が FALSE) の場合に発生。
   */
   ...
   * @throws PHI_IOException コピー元、またはコピー先のディレクトリパスが不正な場合に発生。
   */
  public static function copy($from, $to, array $options = array())
  {
    $array = array();
    $array['recursive'] = PHI_ArrayUtils::find($options, 'recursive', FALSE);
    $array['hidden'] = PHI_ArrayUtils::find($options, 'hidden', FALSE);
    $array['pattern'] = PHI_ArrayUtils::find($options, 'pattern');
   ...
   *
   * @param string $path 対象パス。絶対パス、あるいは APP_ROOT_DIR からの相対パスが有効。
   * @throws PHI_IOException 対象ファイルが存在しない場合に発生。
   */
