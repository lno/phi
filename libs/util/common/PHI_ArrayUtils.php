...
class PHI_ArrayUtils
{
 ...
 /**
   ...
   * PHI_ArrayUtils::replaceValueWithCallback($array, 'bar.baz', function($match) {
   *   return '*';
   * });
   * </code>
   *
  ...
  {
    // 対象キーを検索するロジックは PHI_ArrayUtils::removeKey() と同じ
    $split = explode('.', $name);
    $result = FALSE;
   ...
   * <code>
   * $string = 'foo[bar]';
   * PHI_ArrayUtils::build($string, 'baz'); // array('foo' => 'bar' => 'baz'))
   *
   * $string = 'foo.bar';
   * PHI_ArrayUtils::build($string, 'baz'); // array('foo' => 'bar' => 'baz'))
   * </code>
...
//      $build = $_array0; // TODO TET
    ...
    if (is_array($mergeArray)) {
      $mergeArray = PHI_ArrayUtils::merge($mergeArray, $build);
  ...
  public static function merge(array $array1, array $array2)
  {
    foreach($array2 as $key => $value){
      if (isset($array1[$key]) && is_array($value)){
        $array1[$key] = PHI_ArrayUtils::merge($array1[$key], $value);
   ...
   *
   * // array('bar' => 'baz');
   * $value = PHI_ArrayUtils::find($array, 'foo');
   *
   * // 'baz'
   * $value = PHI_ArrayUtils::find($array, 'foo.bar');
   * </code>
   *
   ...
   *
   * // 100
   * $value = PHI_ArrayUtils::firstValue($array);
   *
   ...
   *
   * // 300
   * $value = PHI_ArrayUtils::lastValue($array);
   *
   ...
   *
   * @param array $array 対象となる配列。
   * @param string $key 変換フォーマット。PHI_ArrayUtils::CONVERT_TYPE_* 定数を指定。
      ...
      foreach ($array as $key => $value) {
        ...
        switch ($convertType) {
          ...
          case self::CONVERT_TYPE_CAMELCAPS:
            $key = PHI_StringUtils::convertCamelCase($key);
            break;
   ...
   * @param int $type PHI_ArrayUtils::APPEND_STRING_* 定数を指定。ビット和形式で指定することも可能。
   *   既定値は PHI_ArrayUtils::APPEND_STRING_BEFORE|PHI_ArrayUtils::APPEND_STRING_AFTER。
   ...
   *   $type = PHI_ArrayUtils::APPEND_STRING_BEFORE|PHI_ArrayUtils::APPEND_STRING_AFTER;
   *
   *   $array = PHI_ArrayUtils::appendEachString($array, '"', $type, $options);
   *
   *   // "foo", "bar", "baz", NULL
   *   echo implode(', ', $array);
   * </code>
   ...
   * <code>
   * // 2.5
   * PHI_ArrayUtils::average(array(1, 2, 3, 4));
   * </code>
   *
   ...
   * //   array(7, NULL, NULL)
   * // );
   * $value = PHI_ArrayUtils::matrix($array);
   * </code>
   *
   * @param array $array 対象となる配列。
