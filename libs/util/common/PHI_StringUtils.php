...
class PHI_StringUtils
{
 ...
 /**
   ...
   * @param int $type 生成する文字列のタイプを PHI_StringUtils::STRING_CASE_* 定数で指定。
   *   PHI_StringUtils::STRING_CASE_LOWER|PHI_StringUtils::STRING_CASE_UPPER|PHI_StringUtils::STRING_CASE_NUMERIC を指定した場合は英数字の組み合わせとなる。
   ...
   * $string = PHI_StringUtils::buildHash('Hello world!');
   * </code>
   *
   * @param string $string 対象となる文字列。
  ...
  {
    $hash = '';
    $secretKey = PHI_Config::getApplication()->getString('secretKey');
   ...
   *   - TRUE: foo_bar…PHI_StringUtils::convertPascalCase('foo_bar') は 'FooBar' を返す
   *   - FALSE: foo_bar…PHI_StringUtils::convertPascalCase('foo_bar') は 'FooBar' を返す
   *
   * string が 'Foo_Bar' の場合 (アンダースコアを含む)
   *   - TRUE: foo__bar…PHI_StringUtils::convertPascalCase('foo__bar') は 'Foo_Bar' を返す
   *   - FALSE: foo_bar…PHI_StringUtils::convertPascalCase('foo_bar') は 'FooBar' を返す (逆変換が不可能となる)
   ...
   * <code>
   * // 'hello WORLD!'
   * $string = PHI_StringUtils::convertSwapCase('HELLO world!');
   * </code>
   *
   * @param string $string 対象となる文字列。
   * @return string 変換後の文字列を返します。
   ...
   * <code>
   * // 2
   * $count = PHI_StringUtils::countMatches('Hello world!', 'o');
   * </code>
   *
   * @param string $string 対象となる文字列。
   ...
   *   - 配列が指定された場合: 返されるデータは {@link PHI_HTMLEscapeArrayDecorator} によってラッピングされたオブジェクトを返します。
   *   - オブジェクトが指定された場合: 返されるデータは {@link PHI_HTMLEscapeObjectDecorator} によってラッピングされたオブジェクトを返します。
   *   配列の中にネスト構造で格納された別の配列やオブジェクトについても、{@link PHI_HTMLEscapeDecorator} によって自動的にラッピングされた値が返されます。
    ...
    $encoding = NULL)
  {
    ...
    if (is_string($values)) {
      ...
      $result = new PHI_HTMLEscapeArrayDecorator($values);

    } else if (is_object($values)) {
      $result = new PHI_HTMLEscapeObjectDecorator($values);
  ...
  public static function formatJSONString($json, $linefeed = "\n", $whitespace = ' ', $indent = 2)
  {
    ...
    $callback = function($array, $arrayType, &$buffer, &$currentIndent)
      ...
      foreach ($array as $name => $value) {
        if (is_array($value)) {
          ...
          if (PHI_ArrayUtils::isAssoc($value)) {
            $buffer .= '{';
    ...
    if (PHI_ArrayUtils::isAssoc($array)) {
      $arrayType = 'assoc';
    } else {
      $arrayType = 'array';
    }
