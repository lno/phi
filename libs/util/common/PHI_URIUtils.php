...
class PHI_URIUtils
{
  ...
  public static function parsePathFromString($string, $base = NULL, $includes = NULL, $target = 'a')
  {
    ...
    for ($i = 0; $i < $j; $i++) {
      ...
      if ($base !== NULL) {
        if (!PHI_URIUtils::isAbsoluteURI($uri)) {
          $uri = PHI_URIUtils::buildAbsoluteURI($base, $uri);
        }
   ...
   *   未指定時は全てのアンカーを取得。
   * @param string $target {@link PHI_URIUtils::parsePathFromString()} 関数を参照。
   * @return array 解析された全てのアンカーを返します。
   */
  public static function parsePathFromURI($uri, array $includes = NULL, $target = 'a')
  {
    $sender = new PHI_HttpRequestSender($uri);
    $parser = $sender->send();

    $fromEncoding = $parser->getEncoding();
    $toEncoding = PHI_Config::getApplication()->get('charset.default');
    ...
    $array = PHI_URIUtils::parsePathFromString($contents, $uri, $includes, $target);
  ...
  public static function buildQueryAssoc($query)
  {
    ...
    if (PHI_StringUtils::nullOrEmpty($query)) {
      return $split;
    }
