 ...
 */
class PHI_DocumentMakeHelper extends PHI_Helper
{
  /**
   * @var string
   */
  ...
  public function linkElement($element, $label = NULL)
  {
    ...
    // URL へのリンク
      ...
      if ($label === NULL) {
        $value = preg_replace(PHI_URLValidator::URL_QUERY_PATTERN, '<a href="\\1">\\1</a>', $element);

      } else {
        $value = preg_replace(PHI_URLValidator::URL_QUERY_PATTERN,
      ...
      if ($preOffset === FALSE && preg_match('/^\s*[-o\*] +(.*)/', $line, $matches, PREG_OFFSET_CAPTURE)) {
        ...
        if ($isList) {
          ...
          if (preg_match('/<(code|pre)>/', $line, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            $line = substr($line, 0, $pos + 6) . PHI_StringUtils::escape(substr($line, $pos + 6));
          ...
          if ($preOffset !== FALSE) {
            ...
            if (preg_match('/<\/(code|pre)>/', $line, $matches, PREG_OFFSET_CAPTURE)) {
              ...
              $line = PHI_StringUtils::escape($before) . substr($line, $pos);
              $preOffset = FALSE;

            } else if (!$preStart) {
              // <code|pre> タグ内からインデント目的の余白を削除
              if (preg_match($regexp, $line, $matches)) {
                $line = PHI_StringUtils::escape($matches[1]);
              }
            }
  ...
  public function sanitize($code)
  {
    ...
    $string = PHI_StringUtils::escape($code, ENT_QUOTES, TRUE, $allows);
