 ...
 * @package net.mail
 */
class PHI_SMTP extends PHI_Object
{
  /**
   * {@link PHI_Socket} オブジェクト。
   * @var PHI_Socket
   */
  private $_socket;
   ...
   * @throws PHI_ConnectException ホストへの接続に失敗した場合に発生。
   */
  public function __construct($host = NULL, $port = 25, $timeout = 30)
  {
    $this->_socket = new PHI_Socket();
    $this->_host = $host;
  ...
  public function connect($host, $port = 25, $timeout = 30)
  {
    ...
    if ($status != 220) {
      ...
      throw new PHI_ConnectException($message);
    }
   ...
   *
   * @throws PHI_CommandException コマンドの実行に失敗した場合に発生。
   */
  public function sendEhlo()
  {
    ...
    if ($this->_status != 250) {
      throw new PHI_CommandException(implode($this->_parameters, $this->_lineFeed));
    }
  }
  ...
  public function sendQuit()
  {
    ...
    if ($this->_status != 221) {
      throw new PHI_ConnectException(implode($this->_parameters, $this->_lineFeed));
    }
  }
