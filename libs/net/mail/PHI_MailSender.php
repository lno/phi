 ...
 * mail:
 *   # 送信手段。
 *   #   - smtp: SMTP プロトコルによるメール送信を行なう。({@link PHI_MailSender::BACKEND_TYPE_SMTP})
 *   #   - sendmail: sendmail プログラムによるメール送信を行なう。({@link PHI_MailSender::BACKEND_TYPE_SENDMAIL})
 *   #   - mail: PHP の mail() 関数によるメール送信を行なう。({@link PHI_MailSender::BACKEND_TYPE_MAIL})
 *   type: smtp
 *
 *   # 件名や本文、添付ファイル名に含まれる半角カナを全角カナに変換する。
 *   hanToZen: TRUE
 *
 *   # 添付ファイル名のエンコード形式。PHI_MailSender::ATTACHMENT_FILENAME_FORMAT_* 定数を指定。
 *   fileNameFormat: <?php echo PHI_MailSender::ATTACHMENT_FILENAME_FORMAT_RFC2231 ?>
 *
 ...
 * @todo SMTP-AUTH 対応
*/
class PHI_MailSender extends PHI_Object
{
  /**
  ...
  /**
   * {@link PHI_MailPart} オブジェクト。
   * @var PHI_MailPart
   */
  protected $_part;

  /**
   * {@link PHI_MailPart} オブジェクト。(HTML パート)
   * @var PHI_MailPart
   */
  protected $_htmlPart;

  /**
   * {@link PHI_MailPart} オブジェクト。(添付パート)
   * @var PHI_MailPart
   */
   ...
   * <code>
   * $mail = new PHI_MailSender(array('type' => 'smtp', 'timeout' => 3));
   * </code>
   */
  public function __construct(array $options = array())
  {
    ...
    $this->_part = new PHI_MailPart($options->getString('linefeed'));
  }
   ...
   * @return PHI_MailSender PHI_MailSender オブジェクトを返します。
   */
  ...
  {
    $value = PHI_StringUtils::replaceLinefeed($value, $this->_options->getString('linefeed'));
  ...
  {
    $subject = PHI_StringUtils::replaceLinefeed($subject, ' ');
  ...
  {
    $body = PHI_StringUtils::replaceLinefeed($body, $this->_options->getString('linefeed'));
   ...
   * <code>
   * $parameters = array('nickname' => 'Guest', 'code' => PHI_StringUtils::buildRandomString());
   ...
   *   {@link PHI_AppPathManager::buildAbsolutePath()} も参照。
   * @param array $parameters テンプレートに割り当てる変数のリスト。
   * @return PHI_MailSender PHI_MailSender オブジェクトを返します。
   * @throws PHI_IOException テンプレートが見つからない場合に発生。
   * @see PHI_MailSender::setTemplateData()
   */
  public function setTemplate($path, array $parameters = array())
  {
    ...
    $path = PHI_AppPathManager::buildAbsolutePath($templatesPath, $path, $extension);
    ...
    if (is_file($path)) {
      ...
      $data = $view->fetch();
      $data = PHI_StringUtils::replaceLinefeed($data, $linefeed);
   ...
   * @return PHI_MailSender PHI_MailSender オブジェクトを返します。
   * @throws PHI_ParseException データの書式が不正な場合に発生。
   * @see PHI_MailSender::setTemplate()
   */
  ...
  public function setHTMLText($htmlText)
  {
    // 改行コードの統一
    $linefeed = $this->_options->getString('linefeed');
    $htmlText = PHI_StringUtils::replaceLinefeed($htmlText, $linefeed);
    ...
    $htmlText = PHI_StringUtils::encodeQuotedPrintable($htmlText, $this->_options->getString('linefeed'), $this->_width);

    // HTML パートの生成
    $encoding = $this->_options->getString('encoding');

    $htmlPart = new PHI_MailPart($linefeed);
   ...
   * @return PHI_MailSender PHI_MailSender オブジェクトを返します。
   * @throws exception PHI_DataNotFoundException ファイルパスが見つからない場合に発生。
   */
    ...
    $contentDisposition = 'attachment')
  {
    ...
    $attachmentPart = new PHI_MailAttachment($this->_options->getString('linefeed'));
    ...
    if ($this->_attachmentPart === NULL) {
      $this->_attachmentPart = new PHI_MailPart($this->_options->getString('linefeed'));
    }
   ...
   * @param string $body メールの本文。
   * @return PHI_MailSender PHI_MailSender オブジェクトを返します。
   * @see {@link PHI_MailSender::setFrom()}
   * @see {@link PHI_MailSender::setRecipients()}
   * @see {@link PHI_MailSender::setBody()}
   * @see {@link PHI_MailSender::send()}
   */
   ...
   *
   * @return PHI_MailSender PHI_MailSender オブジェクトを返します。
   * @throws PHI_ConnectException SMTP サーバとの通信時に例外が起きた場合に発生。
   * @throws PHI_CommandException SMTP サーバへコマンド送信が失敗した場合に発生。
   */
  public function send()
  {
    ...
    if ($this->_hasBcc) {
      if (!$part->hasHeader('To') && !$part->hasHeader('Bcc')) {
        throw new PHI_ParseException('To or Bcc header is undefined.');
      }
    }
    ...
    if ($this->_options->getString('type') == self::BACKEND_TYPE_SMTP) {
      ...
      if ($send) {
        if ($this->_smtp === NULL || !$this->_options->getBoolean('persist')) {
          $smtp = new PHI_SMTP($this->_options->getString('host'),
   ...
   * @throws exception PHI_ParseException メールヘッダが不正な場合に発生。
   */
  private function buildBody()
  {
    ...
    // MIME 形式のメールを生成
      ...
      // テキストパートの生成
      $plainPart = new PHI_MailPart($this->_options->getString('linefeed'));
      ...
      if (strcmp($contentType, 'multipart/mixed') == 0) {
        ...
        if ($this->_htmlPart === NULL) {
          ...
          $alternativePart = new PHI_MailPart($this->_options->getString('linefeed'));
   ...
   * @throws PHI_ParseException 宛先が不明な場合に発生。
   */
  ...
  private function buildHeader()
  {
    ...
    foreach ($headers as $name => $value) {
      $header = $name . ': ' . $value;
      $buildHeader .= PHI_MailPart::splitHeader($header, $width, $lineFeed) . $lineFeed;
    }
