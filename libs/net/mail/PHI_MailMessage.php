 ...
 * @package net.mail
 */
class PHI_MailMessage extends PHI_MailPart
{
  /**
   * メールアドレスリスト定数。
   */
  ...
  {
    $returnPathList = PHI_MailParser::decodeHeader($this, 'return-path');
  ...
  {
    $toList = PHI_MailParser::decodeHeader($this, 'to');
  ...
  {
    $ccList = PHI_MailParser::decodeHeader($this, 'cc');
  ...
  {
    $replyToList = PHI_MailParser::decodeHeader($this, 'reply-to');
  ...
  {
    $fromList = PHI_MailParser::decodeHeader($this, 'from');
  ...
  public function getDate()
  {
    ...
    if (isset($headers['date'])) {
      if (is_array($headers['date'])) {
        $result = PHI_ArrayUtils::lastValue($headers['date']);
      } else {
        $result = $headers['date'];
      }
    }
  ...
  public function getMessageId()
  {
    ...
    if (isset($headers['message-id'])) {
      if (is_array($headers['message-id'])) {
        $result = PHI_ArrayUtils::lastValue($headers['message-id']);
      } else {
  ...
  {
    $subject = PHI_MailParser::decodeHeader($this, 'subject', 0);
  ...
  {
    $splits = PHI_StringUtils::splitExclude($address, ',', '"', FALSE);
    $parseList = array();
    $i = 0;
  ...
  private function splitAddress($address)
  {
    // RFC2822 に準拠
    $separate = array();
    $splits = PHI_StringUtils::splitExclude($address, '<', '"', TRUE);
