 ...
 * @package net.mail
 */
class PHI_MailPart extends PHI_Object
{
  /**
   * メールパートリスト。
   * @var array
   ...
   * MIME パートを追加します。
   *
   * @param PHI_MailPart $parentPart 追加するパート。
   */
  public function addPart(PHI_MailPart $part)
  {
    $this->_parts[] = $part;
  }
   ...
   * MIME パートリストを取得します。
   *
   * @return array PHI_MailPart オブジェクト配列を返します。
   */
   ...
   * メッセージに添付データを追加します。
   *
   * @param PHI_MailAttachment $attachmentFile 追加する添付パート。
   ...
   *
   * @return PHI_MailAttachment PHI_MailAttachment のインスタンスを返します。
   */
  ...
  public function buildPart($width)
  {
    ...
    if ($body !== NULL) {
      ...
      if ($encoding != 'quoted-printable') {
        $body = PHI_StringUtils::wordwrap($body, $width, $lineFeed, $this->getCharset());
      }
