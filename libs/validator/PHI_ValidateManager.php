 ...
 * @package validator
 */
class PHI_ValidateManager extends PHI_Object
{
  /**
   * @var PHI_ActionForm
   */
   ...
   * コンストラクタ。
   *
   * @param PHI_ParameterHolder $validateConfig array バリデータ (behavior) 設定ファイル。
   */
  public function __construct(PHI_ParameterHolder $validateConfig)
  {
    $this->_form = PHI_ActionForm::getInstance();
    $this->_messages = PHI_ActionMessages::getInstance();
   ...
   * $validator = PHI_ValidateManager::createValidator('rangeValidator');
   *
   * // FALSE
   ...
   * @return PHI_Validator バリデータオブジェクトを返します。
  ...
  public static function createValidator($validatorId)
  {
    ...
    $config = PHI_Config::getBehavior()->get($search);

    if ($config) {
      $messages = PHI_ActionMessages::getInstance();
    ...
    if (isset($validateConfig['methods']) || isset($validateConfig['includes']) ||
      ...
      if (isset($validateConfig['includes'])) {
        foreach ($validateConfig['includes'] as $include) {
          $config = PHI_Config::getBehavior($include, TRUE)->get('validate');

          if ($config) {
            $manager = new PHI_ValidateManager($config);
            $manager->execute();
          }
        }
      }
    ...
    if ($groups) {
      ...
      foreach ($keys as $fieldName) {
        ...
        if (is_array($values)) {
          ...
          $files = PHI_FileUploader::getFileInfo($fieldName);
          $fileId = $rowId;
  ...
  private function buildValidator($parentId, array $names, $fieldName, $assocName, $fieldValue)
  {
    if (isset($names[$fieldName]['required'])) {
      ...
      if ($required) {
        $className = 'PHI_RequiredValidator';
        ...
        $validator = new PHI_RequiredValidator(NULL, $holder, $this->_messages);
  ...
  private function checkValidators($parentId, $fieldName, $assocName, $fieldValue, array $validators)
  {
    ...
    $variables = PHI_ArrayUtils::find($fieldAttributes, 'variables', array());
