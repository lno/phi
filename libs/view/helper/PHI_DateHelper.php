 ...
 * 日付・時刻表記の詳しい仕様は {@link PHI_DateUtils::date()} メソッドを参照して下さい。
 *
 * このヘルパは、$date という変数名であらかじめテンプレートにインスタンスが割り当てられています。
 * <code>
 ...
 * @package view.helper
 */
class PHI_DateHelper extends PHI_Helper
{
  /**
   ...
   * </code>
   *
   * @param mixed $time {@link PHI_DateUtils::date()} メソッドを参照。
   * @return string 変換後の文字列を返します。出力フォーマットはヘルパ属性 'timeAgo' を参照。
   */
  public function timeAgo($time)
  {
    $result = FALSE;
    $timestamp = PHI_DateUtils::unixtime($time);
  ...
  public function format($format, $time = NULL)
  {
    ...
    if ($timestamp) {
      ...
      $result = PHI_DateUtils::date($format, $timestamp, $language);
    }
