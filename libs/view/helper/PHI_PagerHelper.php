<?php
/**
 * {@link PHI_Pager} クラスで生成したレコードセットをテンプレート上で表示するためのヘルパメソッドを提供します。
 * このヘルパは、{@link PHI_Pager::assignView()} メソッドがコールされた時点で、テンプレート変数 $pager にページャのインスタンスが割り当てられます。
 *
 * <code>
 ...
 * @package view.helper
 */
class PHI_PagerHelper extends PHI_Helper
{
  /**
   * @var array
   */
    ...
    if ($current) {
      if ($escape) {
        foreach ($current as $key => $value) {
          $current[$key] = PHI_StringUtils::escape($value);
        }
      }
  ...
  public function link($page, $queryData = array())
  {
    ...
    if (!isset($queryData[$ascendingKey]) && !isset($queryData[$descendingKey])) {
      ...
      if (sizeof($orders)) {
        ...
        if (($type = $orders[$key]) == PHI_Pager::SORT_ASCENDING) {
          $value = $ascendingKey;
        } else {
          $value = $descendingKey;
        }
   ...
   * @param string $sortType ソートタイプ。PHI_Pager::SORT_* 定数を指定。
  ...
  public function getSortLabel($label, $name)
  {
    ...
    if (isset($orders[$name])) {
      if ($orders[$name] == PHI_Pager::SORT_ASCENDING) {
  ...
  /**
   * {@link PHI_Pager::addQueryData()} メソッドで追加したクエリデータを元に hidden フィールドを生成します。
   *
