 ...
 * form:
 *   # ヘルパクラス名。
 *   class: PHI_FormHelper
 *
 *   # エラーの発生しているフィールド単位でエラーメッセージを表示する場合は TRUE を指定。
 *   # FALSE を指定した場合はフィールド単位でのエラーが表示されなくなります。
 *   # 全てのエラーメッセージをリスト形式で表示する場合は {@link PHI_HTMLHelper::errors()} メソッドを使用して下さい。
 *   error: TRUE
 *
 *   # エラーが含まれている場合に出力するメッセージ。{@link PHI_FormHelper::containErrors()} メソッド使用時に参照されます。
 ...
 *
 *   # 対象フィールドが必須入力であることを示すシンボル。{@link PHI_FormHelper::inputText()} メソッドを参照。
 ...
 * @package view.helper
 */
class PHI_FormHelper extends PHI_Helper
{
  /**
   * @var PHI_ActionForm
   */
  ...
  /**
   * フォームの終了タグを生成します。
   * {@link PHI_AuthorityUser::saveToken() トランザクショントークン} が発行されている場合は、自動的にトークン用の hidden タグを生成します。
   *
   * @return string 生成したタグを返します。
   */
  ...
  public function label($fieldId, $label, $attributes = array())
  {
    ...
    $buffer = sprintf('<label%s>%s</label>',
      $attributes,
      PHI_StringUtils::escape($label));

    return $buffer;
  }

  /**
   * {@link PHI_ActionForm::get()} メソッドに {@link PHI_StringUtils::escape() HTML エスケープ} 機能を追加した拡張メソッドです。
   *
  ...
  /**
   * {@link PHI_ActionForm::hasName()} のエイリアスメソッドです。
   */
  ...
  /**
   * {@link PHI_ActionForm::getFields()} メソッドに {@link PHI_StringUtils::escape() HTML エスケープ} 機能を追加した拡張メソッドです。
   *
   * @param bool $escape 値を HTML エスケープした状態で返す場合は TRUE を指定。
   */
  public function getFields($escape = TRUE)
  {
    ...
    if ($escape) {
      $values = PHI_StringUtils::escape($values);
    }
  ...
  /**
   * {@link PHI_ActionMessages::hasFieldError()} メソッドを複数のフィールドチェックに対応させた拡張メソッドです。
   *
  ...
  /**
   * {@link PHI_ActionMessages::getFieldError()} メソッドに {@link PHI_StringUtils::escape() HTML エスケープ} 機能を追加した拡張メソッドです。
   *
   * @param bool $escape 値を HTML エスケープした状態で返す場合は TRUE を指定。
   */
  public function getFieldError($name, $escape = TRUE)
  {
    ...
    if ($escape) {
      $message = PHI_StringUtils::escape($message);
    }
  ...
  private function decorateControlAppendError(&$buffer, $fieldName, $extra)
  {
    ...
    if ($this->_config->getBoolean('error', TRUE)) {
      if (PHI_ArrayUtils::find($extra, 'error', TRUE)) {
        $isAppendError = TRUE;
      }

    } else if (PHI_ArrayUtils::find($extra, 'error', FALSE)) {
      $isAppendError = TRUE;
    }
  ...
  private function decorateControlAppendTitle(&$buffer, $type, $fieldName, $attributes, $extra)
  {
    ...
    if ($type === 'radio' || $type === 'checkbox' || $type === 'set') {
      if (isset($extra['label']) && $extra['label']) {
        $label = PHI_StringUtils::escape($extra['label']);
   ...
   *
   * @see PHI_FormHelper::inputText()
   * @see PHI_FormHelper::inputTextFull()
   */
  ...
  public function selectNumber($fieldName, $options, $attributes = array(), $extra = array())
  {
    ...
    $from = PHI_ArrayUtils::find($options, 'from', 0);
    $to = PHI_ArrayUtils::find($options, 'to', 0);
    $interval = PHI_ArrayUtils::find($options, 'interval', 1);
   ...
   * @param mixed $attributes 各フィールドに共通して追加する属性。属性。{@link PHI_HTMLHelper::link()} メソッドを参照。
   * @param mixed $extra {@link inputText()} メソッドを参照。
   * @return string 生成したタグを返します。
   */
    ...
    $extra = array())
  {
    ...
    $time = PHI_ArrayUtils::find($conditions, 'time', time());
    $fieldPrefix = PHI_ArrayUtils::find($conditions, 'fieldPrefix', 'date');
    $fieldAssoc = PHI_ArrayUtils::find($conditions, 'fieldAssoc', TRUE);

    $yearAsText = PHI_ArrayUtils::find($conditions, 'yearAsText', FALSE);
    $yearEmpty = PHI_ArrayUtils::find($conditions, 'yearEmpty');
    $yearFormat = PHI_ArrayUtils::find($conditions, 'yearFormat', 'Y');
    $yearValueFormat = PHI_ArrayUtils::find($conditions, 'yearValueFormat', 'Y');

    // PHI_ArrayUtils::find() は戻り値が第 3 引数の型に変換されるため文字列型のデフォルト値を設定
    // ('+3' という指定が数値型で認識されると 3 年～と誤認されるため)
    $startYear = PHI_ArrayUtils::find($conditions, 'startYear', '1900'); // 文字列型、または数値型
    $endYear = PHI_ArrayUtils::find($conditions, 'endYear', date('Y')); // 文字列型、または数値型

    $displayYears = PHI_ArrayUtils::find($conditions, 'displayYears', TRUE);
    $reverseYears = PHI_ArrayUtils::find($conditions, 'reverseYears', TRUE);

    $monthEmpty = PHI_ArrayUtils::find($conditions, 'monthEmpty');
    $monthFormat = PHI_ArrayUtils::find($conditions, 'monthFormat', 'm');
    $monthValueFormat = PHI_ArrayUtils::find($conditions, 'monthValueFormat', 'm');
    $displayMonths = PHI_ArrayUtils::find($conditions, 'displayMonths', TRUE);

    $dayEmpty = PHI_ArrayUtils::find($conditions, 'dayEmpty');
    $dayFormat = PHI_ArrayUtils::find($conditions, 'dayFormat', 'd');
    $dayValueFormat = PHI_ArrayUtils::find($conditions, 'dayValueFormat', 'd');
    $displayDays = PHI_ArrayUtils::find($conditions, 'displayDays', TRUE);
    $separator = PHI_ArrayUtils::find($conditions, 'separator', array('/', '/'));
    $tagSeparator = PHI_ArrayUtils::find($conditions, 'tagSeparator');
    ...
    for ($i = 0; $i < $j; $i++) {
      $separators[$i] = str_replace('\1', PHI_StringUtils::escape($separator[$i]), $this->_config->getString('fieldSeparatorTag'));
    }
    ...
    if ($displayYears) {
      if ($yearAsText) {
        ...
        while (TRUE) {
          ...
          $value = PHI_DateUtils::date($yearValueFormat, $work);
          $output = PHI_DateUtils::date($yearFormat, $work);
    ...
    if ($displayMonths) {
      ...
      for ($i = 1; $i <= 12; $i++) {
        ...
        $value = PHI_DateUtils::date($monthValueFormat, $work);
        $output = PHI_DateUtils::date($monthFormat, $work);
      ...
      if ($this->_form->hasName($fieldNameMonth)) {
        ...
        $selected = PHI_DateUtils::date($monthValueFormat, $time);
      }
    ...
    if ($displayDays) {
      ...
      for ($i = 1; $i <= 31; $i++) {
        ...
        $value = PHI_DateUtils::date($dayValueFormat, $work);
        $output = PHI_DateUtils::date($dayFormat, $work);
      ...
      if ($this->_form->hasName($fieldNameDay)) {
        ...
        $selected = PHI_DateUtils::date($dayValueFormat, $time);
      }
  ...
  public function inputSubmit($value = NULL, $attributes = array(), $extra = array())
  {
    ...
    $fieldName = PHI_ArrayUtils::find($attributes, 'name');
   ...
   * image フィールドを生成します。
   *
   * @param string $source {@link PHI_HTMLHelper::buildAssetPath()} メソッドを参照。
   * @param mixed $attributes タグに追加する属性。{@link PHI_HTMLHelper::link()} メソッドを参照。
   * @param mixed $extra タグの出力オプション。
  ...
  public function textarea($fieldName, $attributes = array(), $extra = array())
  {
    ...
    if ($this->_form->hasName($fieldName)) {
      $value = PHI_StringUtils::escape($this->_form->get($fieldName));

    } else if (isset($attributes['value'])) {
      $value = PHI_StringUtils::escape($attributes['value']);
  ...
  private function buildMultipleFields($type, $fieldName, $options, $attributes, $extra)
  {
    ...
    // フィールド要素のセパレータ
    $separator = PHI_ArrayUtils::find($extra, 'separator');

    if ($separator !== NULL) {
      $separator = "\n" . PHI_StringUtils::escape($separator) . "\n";
    } else {
      $separator = "\n";
    }

    $tagSeparator = PHI_ArrayUtils::find($extra, 'tagSeparator');
    ...
    // リクエスト (またはフォームにセット) されたデフォルト値がない、またはリストが未送信の場合に限りヘルパに渡されたデフォルト値を設定する
      ...
      $selected = PHI_ArrayUtils::find($extra, 'selected');
    }
    ...
    $iterator = function($options)
      ...
      foreach ($options as $optionValue => $optionLabel) {
        ...
        $optionLabel = PHI_StringUtils::escape($optionLabel);
        ...
        if ($type == 'select') {
          ...
          $optionValue = PHI_StringUtils::escape($optionValue);
        ...
        // checkbox、radio の項目生成
          ...
          if (PHI_StringUtils::nullOrEmpty($optionLabel)) {
   ...
   * @return array
   * @see PHI_Helper::constructParameters()
   */
