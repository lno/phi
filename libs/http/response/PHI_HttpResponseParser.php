<?php
/**
 * PHI_HttpRequestSender から発行したリクエストに対応するレスポンスを解析します。
 *
 * @package http.response
 */
class PHI_HttpResponseParser extends PHI_Object
{
  /**
   * 生の応答ヘッダ。
   */
   ...
   * @see PHI_HttpResponseParser::getHeader()
   */
  ...
  public function getContents($convertEncoding = FALSE)
  {
    ...
    if ($contentEncoding === 'gzip' || $contentEncoding === 'deflate') {
      if (!extension_loaded('zlib')) {
        ...
        $contents = PHI_StringUtils::decodeGzip($contents);
      } else {
    ...
    if ($convertEncoding) {
      ...
      $destinationEncoding = PHI_Config::getApplication()->get('charset.default');
   ...
   * @see PHI_HttpResponseParser::getContents()
   */
