 ...
 *
 * このクラスは 'response' コンポーネントとして DI コンテナに登録されているため、{@link PHI_DIContainer::getComponent()}、あるいは {@link PHI_WebApplication::getResponse()} からインスタンスを取得することができます。
 *
 * PHI_HttpResponse が提供するいくつかのメソッドは、{@link flush()} メソッドによってクライアントにレスポンスが返されるタイミングで実行されます。
 *   - {@link setStatus()}
 *   - {@link setHeader()}
 *   - {@link setCacheExpire()}
 ...
 */
class PHI_HttpResponse extends PHI_Object
{
  /**
   * 出力エンコーディング。
   * @var string
  ...
  public function __construct()
  {
    ...
    if ($config->getBoolean('controller.detectUserAgent')) {
      $userAgent = PHI_FrontController::getInstance()->getRequest()->getUserAgent();
   ...
   * ビューオブジェクトを取得します。
   *
   * @return PHI_View ビューオブジェクトを返します。
   */
  public function getView()
  {
    return PHI_DIContainerFactory::getContainer()->getComponent('view');
  }
  ...
  {
    return PHI_ActionMessages::getInstance();
  }
   ...
   *
   * エラーメッセージのテンプレートは {PHI_ROOT_DIR}/skeleton/templates/http_error.php が使用されます。
   ...
   * @param string $version HTTP プロトコルのバージョン。
   * @see PHI_HttpResponse::sendStatus()
   */
  public function sendError($status, $message = NULL, $version = '1.1')
  {
    ...
    if (!file_exists($path)) {
      $path = sprintf('%s/skeleton/templates/http_error.php', PHI_ROOT_DIR);
    }
    ...
    PHI_FrontController::getInstance()->getObserver()->dispatchEvent('preOutput', $arguments);
   ...
   * @param string $version HTTP プロトコルのバージョン。
   * @throws PHI_UnsupportedException サポートされていないステータスコードが指定された場合に発生。
   */
   ...
   * @throws PHI_RequestException ルートが確定していない状態でメソッドをコールした場合に発生。
   */
  public function sendRedirectAction($path, array $queryData = array(), $appendSessionId = TRUE)
  {
    ...
    if ($appendSessionId) {
      $sessionName = PHI_Config::getApplication()->getString('session.name');
   ...
   * @throws PHI_SecurityException URI に CrLf インジェクションとなりうる文字列が含まれる場合に発生。
   * @throws PHI_UnsupportedException サポートされていないステータスコードが指定された場合に発生。
   */
  ...
  public function setDownloadData($data, $name = NULL, $isBinary = FALSE)
  {
    if ($name === NULL) {
      ...
      $name = PHI_StringUtils::convertCamelCase($actionName);
    }
    ...
    // PHI_KernelEventObserver::outputBuffer() の動作に影響
   ...
   * @see PHI_HttpResponse::hasWriteBuffer()
   */
