<?php
require PHI_LIBS_DIR . '/net/agent/PHI_UserAgent.php';

/**
 * クライアントから要求された HTTP リクエストを管理します。
 *
 * このクラスは 'request' コンポーネントとして DI コンテナに登録されているため、{@link PHI_DIContainer::getComponent()}、あるいは {@link PHI_WebApplication::getRequest()} からインスタンスを取得することができます。
 *
 * @package http.request
 */
class PHI_HttpRequest extends PHI_Object
{
  /**
   * HTTP GET リクエスト定数。
   */
   ...
   * セッションオブジェクトを取得します。
   *
   * @return PHI_HttpSession セッションオブジェクトを返します。
   */
  public function getSession()
  {
    return PHI_DIContainerFactory::getContainer()->getComponent('session');
  }
   ...
   * @throws PHI_RequestException クライアントから送信されたパラメータに不正な文字が含まれる場合に発生。
  ...
  private function escapeXSS($value, $encoding = NULL)
  {
    if (is_array($value)) {
      ...
      $value = PHI_StringUtils::replaceLinefeed($value);
      ...
      if ($encoding !== NULL && !mb_check_encoding($value, $encoding)) {
        ...
        throw new PHI_RequestException($message);
      }
    }
   ...
   *
   * @return int PHI_HttpRequest::HTTP_* 定数を返します。
   */
  ...
  /**
   * クライアントから要求された生のリクエストデータを解析し、{@link PHI_ParameterHolder} オブジェクトに変換します。
   ...
   *
   * @param string $method リクエストメソッドの形式。{@link PHI_HttpRequest::HTTP_GET}、{@link PHI_HttpRequest::HTTP_POST} のいずれかを指定。
   * @return PHI_ParameterHolder パラメータホルダのインスタンスを返します。
   */
  public function parseRawData($method = self::HTTP_GET)
  {
    ...
    $parse = PHI_MIME::parse($this->getContentType());

    if ($parse['content'] === 'application/json') {
      $data = json_decode($data, TRUE);

    } else {
      $data = PHI_URIUtils::buildQueryAssoc($data);
    }

    $hash = $this->escapeXSS($data, $this->_inputEncoding);
    $holder = new PHI_ParameterHolder($hash);
   ...
   * リクエストされたルート情報を設定します。
   *
   * @param PHI_Route $route リクエストルート。
   */
  public function setRoute(PHI_Route $route)
  {
    $this->_route = $route;
  }
   ...
   * リクエストされたルート情報を取得します。
   *
   * @return PHI_Route リクエストされたルート情報を返します。
  ...
  private function getRequestValue($data, $name, $alternative, $emptyToAlternative = FALSE)
  {
    if ($name === NULL) {
      ...
      $data = PHI_ArrayUtils::find($data, $name);
   ...
   * @see PHI_HttpRequest::getParameters()
   */
   ...
   * クライアントのホスト名を取得します。
   *
   * @param bool $proxy {@link PHI_HttpRequest::getRemoteAddress()} メソッドを参照。
  ...
  public function getRemoteAddress($ip2long = FALSE, $proxy = FALSE)
  {
    ...
    if ($ip2long) {
      return (string) PHI_NetworkUtils::convertIPToInteger($address);
    }
  ...
  /**
   * {@link PHI_HttpRequest::getReferrer()} へのエイリアスです。
  ...
  public function getUserAgent()
  {
    ...
    if ($adapter === NULL) {
      ...
      $adapter = PHI_UserAgent::getInstance()->getAdapter($userAgent);
    }
  ...
  public function isLocal()
  {
    ...
    if (PHI_NetworkUtils::hasContainNetwork($range, $remoteAddress)) {
      return TRUE;
    }
