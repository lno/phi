 ...
 */
class PHI_HttpRequestSender extends PHI_Object
{
  /**
   * POST データの送信形式。(JSON)
   */
  ...
  /**
   * リクエストメソッド。(PHI_HttpRequest::HTTP_*)
   * @var string
   */
  private $_requestMethod = PHI_HttpRequest::HTTP_GET;
   ...
   * @return PHI_HttpRequestSender PHI_HttpRequestSender オブジェクトを返します。
   */
   ...
   * リクエストメソッドを設定します。
   *
   * @param int $requestMethod {@link PHI_HttpRequest::HTTP_GET}、または {@link PHI_HttpRequest::HTTP_POST} を指定。
   * @return PHI_HttpRequestSender PHI_HttpRequestSender オブジェクトを返します。
   */
  ...
  public function addUploadFile($name, $filePath = NULL, $mimeType = NULL)
  {
    ...
    if ($filePath === NULL) {
      ...
      if (!PHI_FileUtils::isAbsolutePath($filePath)) {
        $filePath = PHI_FileUtils::buildAbsolutePath($filePath);
      }
  ...
  {
    if (PHI_StringUtils::nullOrEmpty($this->_requestPath)) {
    ...
    if ($this->_requestMethod == PHI_HttpRequest::HTTP_GET) {
   ...
   * サーバにリクエストを送信します。
   *
   * @return PHI_HttpResponseParser {@link PHI_HttpResponseParser} のインスタンスを返します。
   * @throws PHI_ConnectException 指定された URI に接続できなかった場合に発生。
   */
  public function send()
  {
    ...
    if (empty($parse['host'])) {
      throw new PHI_ConnectException('Request URI is not set');
    }
    ...
    if (sizeof($parameters)) {
      switch ($this->_requestMethod) {
        case PHI_HttpRequest::HTTP_GET:
          $data = http_build_query($parameters, '', '&');
          break;

        case PHI_HttpRequest::HTTP_POST:

          // マルチパート構成
          if ($this->_postEncoding === self::CONTENT_TYPE_MULTIPART_FORM_DATA) {
            $data = NULL;
            $boundary = str_repeat('-', 20) . PHI_StringUtils::buildRandomString();
    ...
    if (!$hasContentType) {
      if ($this->_requestMethod === PHI_HttpRequest::HTTP_POST) {
    ...
    if (is_array($http_response_header)) {
      return new PHI_HttpResponseParser($http_response_header, $data);
