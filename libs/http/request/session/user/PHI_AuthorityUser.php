 ...
 *
 * PHI_AuthorityUser が管理するセッション情報 (ユーザデータ、ロール、ログイン状態) はモジュール単位で有効となります。
 * このクラスは 'user' コンポーネントとして DI コンテナに登録されているため、{@link PHI_DIContainer::getComponent()}、あるいは {@link PHI_WebApplication::getUser()} からインスタンスを取得することができます。
 *
 * @package http.request.session.user
 */
class PHI_AuthorityUser extends PHI_Object
{
  /**
   * 全てのロールを満たす必要がある。
   */
  ...
  /**
   * {@link PHI_BlowfishCipher} オブジェクト。
   * @var PHI_BlowfishCipher
   */
  private $_cipher;
  ...
  {
    $sessionConfig = PHI_Config::getApplication()->get('session');
    ...
    if ($controller->getRequest()->getRoute()) {
      $this->_namespace = $controller->getRequest()->getRoute()->getModuleName();
    } else {
      // TODO コンソールからの利用を想定する
      $this->_namespace = 'console';
    }
    ...
    if (sizeof($this->_context)) {
      ...
      if ($timeout > 0 && $currentTime - $accessTime >= $timeout) {
        ...
        if (PHI_StringUtils::nullOrEmpty(SID)) {
  ...
  public function hasRole($role = NULL)
  {
    ...
    if (PHI_StringUtils::nullOrEmpty($userRoles)) {
      if (sizeof($userRoles)) {
        return TRUE;
      }
   ...
   * $user->isCurrentActionAuthenticated(PHI_AuthorityUser::REQUIRED_ALL_ROLES)
   * </code>
   *
   * @param bool $requiredRole ロールを所有していると見なす条件。PHI_AuthorityUser::REQUIRED_* 定数を指定。
   ...
   * 発行されたトークン ID は {@link PHI_FormHelper::close()} メソッドをコールすることにより、自動的に hidden タグとしてフォーム内に埋め込まれます。
   *
   ...
   * @return int PHI_AuthorityUser::TOKEN_* 定数を返します。
   */
  ...
  public function login()
  {
    ...
    if (!$this->isLogin()) {
      // Session fixation 対策
      $session = PHI_FrontController::getInstance()->getRequest()->getSession();
      $session->updateId();
