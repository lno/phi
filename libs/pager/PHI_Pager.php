 ...
 * @package pager
 */
abstract class PHI_Pager extends PHI_Object
{
  /**
   * 全レコード取得定数。
   */
  ...
  /**
   * ソートリスト。
   * array(array('対象キー' => PHI_Pager::SORT_ASCENDING|PHI_Pager::SORT_DESCENDING), ...);
   * @var array
   */
  ...
  protected function bindPosition($maxRow)
  {
    ...
    if ($pageInRecord == self::OFFSET_NONE) {
      ...
      // 最終ページの取得
      $this->_lastPage = PHI_NumberUtils::roundUp($maxRow / $pageInRecord, 0);
  ...
  {
    $fields = PHI_ActionForm::getInstance()->getFields();
  ...
  {
    $array = PHI_Config::getHelpers()->get('pager')->toArray();
    ...
    $view = PHI_FrontController::getInstance()->getResponse()->getView();
