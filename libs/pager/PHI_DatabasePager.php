 ...
 *
 *   $pager = new PHI_DatabasePager();
 *
 *   // ページ辺りの表示件数を 10 件に指定
 *   $pager->fetchStatement($conn, $sql, 10);
 *
 *   // 'nickname' キーで昇順ソート
 *   $pager->addSort('nickname', PHI_Pager::SORT_ASCENDING);
 *
 ...
 * }
 * </code>
 * <i>テンプレート上では {@link PHI_DatabasePager ページャヘルパ} を用いてレコードセットを表示することができます。</i>
 *
 * @package pager
 */
class PHI_DatabasePager extends PHI_Pager
{
  /**
   * オプティマイザヒント定数。(MySQL)
   ...
   * @param string $type ソート方法。PHI_Pager::SORT_* 定数を指定。
   * @param int $priority ソートの優先順位。未指定の場合は最も低い優先順位になります。
   */
  public function addSort($column, $type = PHI_Pager::SORT_ASCENDING, $priority = NULL)
  {
    if (!isset($this->_requestSort[$column])) {
      foreach ($this->_orders as $orderPriority => $orders) {
        foreach ($orders as $orderColumn => $orderType) {
          if (strcasecmp($column, $orderColumn) === 0) {
            PHI_ArrayUtils::removeShift($this->_orders, $orderPriority);
            break;
          }
        }
      }
      ...
      if ($priority !== NULL) {
        PHI_ArrayUtils::insert($this->_orders, $order, $priority);
      } else {
        $this->_orders[] = $order;
      }
    }
   ...
   *
   * @param PHI_DatabaseStatement $statement ステートメントオブジェクト。
   * @param int $pageInRecord 1 ページ辺りに表示するレコード件数。{@link PHI_Pager::OFFSET_NONE} 指定時は全件を取得します。
   */
  public function fetch(PHI_DatabaseStatement $statement, $pageInRecord = 10)
  {
    $this->fetchStatement(
   ...
   * @param int $pageInRecord 1 ページ辺りに表示するレコード件数。{@link PHI_Pager::OFFSET_NONE} 指定時は全件を取得します。
   */
  public function fetchStatement(PHI_DatabaseConnection $connection, $query, $pageInRecord = 10)
  {
  ...
  /**
   * @param PHI_DatabaseConnection $connection
   * @param $query
   * @return array
   */
  private function fetchStatementWithMySQL(PHI_DatabaseConnection $connection, $query)
  {
    ...
    if ($this->_pageInRecord !== self::OFFSET_NONE) {
      // 不正なページ指定時
      $maxPage = PHI_NumberUtils::roundUp($maxRow / $this->_pageInRecord, 0);
   ...
   */
  private function getRecordCount(PHI_DatabaseConnection $connection, $query)
  {
   ...
   */
  private function buildRecordSet(PHI_DatabaseConnection $connection, $query)
  {
