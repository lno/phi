 ...
 * ステートメントは {@link PHI_DatabaseConnection::createStatement()} メソッドで作成することができます。
 *
 * <code>
 ...
 * @package database
 */
class PHI_DatabaseStatement extends PHI_Object
{
  /**
   * フェッチタイプ定数。
   ...
   * @var string
   */
  protected $_resultSetClassName = 'PHI_DatabaseResultSet';
   ...
   * @var string
   */
  private $_mappingClassName = 'PHI_RecordObject';
   ...
   */
  public function __construct(PHI_DatabaseConnection $connection, $query)
  {
   ...
   */
  public function setFetchMode($fetchType, $mappingClassName = 'PHI_RecordObject')
  {
   ...
   * @param array $bindValues バインド変数の配列。
   *   名前付きプレースホルダの場合は array(':{name}' => '{value}') 形式、疑問符プレースホルダを使用する場合は array('{value1}', '{value2}'...) 形式でパラメータを指定します。
   * @return PHI_DatabaseResultSet クエリを実行した結果のレコードセットを返します。
   */
   ...
   * @return mixed $returnAffectedCount が TRUE の場合は作用したレコード数、FALSE の場合は PHI_DatabaseResultSet を返します。
   */
  private function executeStatement(array $bindValues, $returnAffectedCount)
  {
    ...
    $profiler = PHI_DatabaseManager::getInstance()->getProfiler();
  ...
  {
    $parser = new PHI_SQLPreparedStatementParser($this->_query);
   ...
   * コネクションオブジェクトを取得します。
   *
   * @return PHI_DatabaseConnection コネクションオブジェクトを返します。
   */
