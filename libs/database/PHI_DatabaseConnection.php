 ...
 * コネクションは {@link PHI_DatabaseManager::getConnection()}、あるいは {@link PHI_DatabaseManager::getConnectionWithConfig()} メソッドから生成することができます。
 *
 * @package database
 */
class PHI_DatabaseConnection extends PHI_Object
{
  /**
   * ステートメントクラス名。
   * @var string
   */
  protected $_statementClassName = 'PHI_DatabaseStatement';
   ...
   *
   * @return PHI_DatabaseTransactionController トランザクションコントローラを返します。
   */
   ...
   * @return PHI_DatabaseStatement PHI_DatabaseStatement のインスタンスを返します。
   */
   ...
   * @return PHI_DatabaseResultSet PHI_DatabaseResultSet のインスタンスを返します。
   */
   ...
   *
   * @return PHI_DatabaseCommand データベースに対応するオペレータオブジェクトを返します。
   *   例えば MySQL を使用している場合は、{@link PHI_DatabaseMySQLCommand} オブジェクトを返します。
   * @throws PHI_UnsupportedException オペレータがデータベースドライバに対応していない場合に発生。
   */
  public function getCommand()
  {
    ...
    switch ($driverName) {
      case 'mysql':
        $command = new PHI_DatabaseMySQLCommand($this);
        break;

      case 'pgsql':
        $command = new PHI_DatabasePostgreSQLCommand($this);
        break;
  ...
  {
    $this->close();
    $this->_adapter = new PDO($this->_dsn, $this->_user, $this->_password, $this->_options);
  }

  /**
   * 現在有効なコネクションを閉じます。
   * データベースへの持続接続が有効な場合、実際には接続が閉じられず、メモリ領域が開放される点に注意して下さい。
   * {@link PHI_PDOConnector} で開いた全てのコネクションを閉じる場合は {@link PHI_DatabaseManager::closeAll()} メソッドを使用して下さい。
   *
   */
