 ...
 */
abstract class PHI_DatabaseCommand extends PHI_Object
{
  /**
   * データベースコネクションオブジェクト。
   ...
   * コンストラクタ。
   *
   * @param PHI_DatabaseConnection $connection PHI_DatabaseConnection オブジェクト。
   */
  public function __construct(PHI_DatabaseConnection $connection)
  {
   ...
   * これは、このメソッドの動作、メソッド名、ここで書かれていること全てが phi の将来のバージョンで予告な>く変更される可能性があることを意味します。
  ...
  {
    $holder = new PHI_ParameterHolder($data, TRUE);
  ...
  /**
   * PHI_DatabaseCommand がサポートするデータ型から、データベースのネイティブ型を取得します。
   *
   * @param string $type PHI_DatabaseCommand がサポートするデータ型。
  ...
  public function importCSV($tableName, $path, array $options = array())
  {
    ...
    $holder = new PHI_ParameterHolder($options);
    ...
    while (($data = fgets($fp)) !== FALSE) {
      ...
      if (strlen(trim($data)) && substr($data, 0, 1) !== '#') {
        $data = PHI_StringUtils::splitExclude($data, $delimiter, $enclosure, FALSE, $dataEncoding);
        $data = PHI_ArrayUtils::trim($data, NULL, TRUE, TRUE, FALSE, $dataEncoding);
        $i++;
        ...
        foreach ($data as &$field) {
          ...
          if (mb_substr($field, 0, 1, $dataEncoding) === $enclosure && mb_substr($field, $length - 1, 1, $dataEncoding) === $enclosure) {
            ...
            $field = new PHI_DatabaseExpression($field);
          }
        }
  ...
  public function insert($tableName, array $data, $name = NULL)
  {
    ...
    $query = sprintf('INSERT INTO %s(%s) VALUES(%s)',
      $tableName,
      implode(', ', $columnNames),
      implode(', ', $this->quoteValues($columnValues)));
   ...
   *   $data = array(
   *     array(1, 'en', 'Hello', new PHI_DatabaseExpression('NOW()'));
   *     array(2, 'jp', 'こんにちは', new PHI_DatabaseExpression('NOW()'));
   *   );
   *
