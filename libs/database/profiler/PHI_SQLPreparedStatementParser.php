 ...
 */
class PHI_SQLPreparedStatementParser extends PHI_Object
{
  /**
   * 名前付きプレースホルダ。
   * @var int
  ...
  {
    if (PHI_StringUtils::searchIndex($preparedStatement, ':', 0, '\'') !== FALSE) {
      return self::PLACE_HOLDER_TYPE_NAME;

    } else if (PHI_StringUtils::searchIndex($preparedStatement, '?', 0, '\'') !== FALSE) {
   ...
   * @throws PHI_ParseException 予測されないバインド変数が割り当てられた場合に発生。
   */
  private function bindByParameterName($statement, array $bindVariables)
  {
    ...
    while (($pos = PHI_StringUtils::searchIndex($statement, ':', 0, '\'')) !== FALSE) {
      ...
      if (preg_match('/:[\w]+/', mb_substr($statement, $pos, $length, $encoding), $matches)) {
        if (array_key_exists($matches[0], $bindVariables)) {
          ...
          $statement = PHI_StringUtils::replaceSubstring($statement, $statementValue, $pos, strlen($matches[0]));
   ...
   * @param array $bindVariables バインド変数リスト。
   * @return array バインド変数を展開したステートメントを返します。
   * @throws PHI_ParseException ステートメントのプレースホルダにバインドされた変数の数がマッチしない場合に発生。
   */
  private function bindByInterrogation($statement, array $bindVariables)
  {
    ...
    while (($pos = PHI_StringUtils::searchIndex($statement, '?', 0, '\'')) !== FALSE) {
      ...
      $statement = PHI_StringUtils::replaceSubstring($statement, $statementValue, $pos, 1);
