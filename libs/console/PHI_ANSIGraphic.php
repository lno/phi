 ...
 * @package console
 */
class PHI_ANSIGraphic extends PHI_Object
{
  /**
   * 文字色定数。(黒)
   */
   ...
   * $string = PHI_ANSIGraphic::build('Hello world!', PHI_ANSIGraphic::FOREGROUND_RED|PHI_ANSIGraphic::ATTRIBUTE_UNDERSCORE);
   * </code>
   * このメソッドは Windows に対応していないため、Windows 環境下ではオリジナルの文字列を返します。
   *
   * @param string $string 対象となる文字列。
   * @param int $graphicMode 文字列に設定するカラー定数。
   *   {@link PHI_ANSIGraphic} クラスが提供する FOREGROUND_*、BACKGROUND_*、ATTRIBUTE_* 定数の組み合わせが指定可能。
