 ...
 * @package message
 */
class PHI_ActionMessages extends PHI_Object
{
  /**
   ...
   *
   * @return PHI_ActionMessages メッセージのインスタンスオブジェクトを返します。
   */
   ...
   * フィールドに設定したメッセージは {@link getFieldError()} や、テンプレート上からは {@link PHI_FormHelper::getFieldError()} といったメソッドで取得することができます。
   *
   ...
   *
   * @param int $type PHI_ActionMessages::ERROR_TYPE_* 定数を指定。
   * @return int メッセージオブジェクトに設定されている総エラーメッセージ数を返します。
   *   type が未指定の場合は {@link PHI_ActionMessages::ERROR_TYPE_DEFAULT} + {@link PHI_ActionMessages::ERROR_TYPE_FIELD} の総数を返します。
   */
   ...
   *   type が未指定の場合は {@link PHI_ActionMessages::ERROR_TYPE_DEFAULT} + {@link PHI_ActionMessages::ERROR_TYPE_FIELD} のリストを返します。
   */
