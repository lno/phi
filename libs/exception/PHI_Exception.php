<?php
/**
 * phi から発生する可能性がある例外の基底クラスです。
 * フレームワークのコアライブラリが全て PHI_Exception の子クラスの例外をスローする訳ではない点に注意して下さい。
 ...
 * 独自の例外クラスを作成する場合は {@link PHI_Exception} を基底クラスとすることを推奨します。
 * また、新しい例外クラスを作成する際は、SPL 例外に類似するクラスがないか確認しておくべきです。
 *
 * @package exception
 */
class PHI_Exception extends Exception
{
  /**
   * トリガーコード定数。(PHP)
   */
  ...
  {
    if ($message === NULL) {
      // PHI_xxxException から 'xxx' の要素名を取得
